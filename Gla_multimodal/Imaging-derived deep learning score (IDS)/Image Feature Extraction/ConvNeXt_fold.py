...
fold = '/storage0/lab/khm1576/IDPs/OCT/{fold_name}'
...
print(job_index,"B_scan convnext_femto ")

class FocalLoss(nn.Module):
    def __init__(self, alpha=3, gamma=2, reduction='mean'):
...
class ModifiedConvNext(nn.Module):
    def __init__(self, pretrained=True):
        ...
        self.convnext = timm.create_model('convnext_femto', pretrained=pretrained)
        
        
        in_features = self.convnext.head.fc.in_features

        
        self.convnext.head.drop = nn.Dropout(p=0.4)

        
        self.convnext.head.fc = nn.Linear(in_features=in_features, out_features=1)

    def forward(self, x):
        return self.convnext(x)


ConvNext = ModifiedConvNext(pretrained=True).to(Device)

for param in ConvNext.parameters():
            param.requires_grad = True

optimizer_ft = optim.AdamW(filter(lambda p: p.requires_grad, ConvNext.parameters()), eps=1e-8, lr=0.0001, weight_decay=1e-4)
...
def train_ConvNext(model, criterion, optimizer, scheduler, dataloaders, dataset_sizes, num_epochs=50, early_stopping_patience=5):
    since = time.time()
...
ConvNext = train_ConvNext(ConvNext, criterion, optimizer_ft, scheduler, dataloaders, dataset_sizes, num_epochs=50, early_stopping_patience=5)


torch.save(ConvNext, f'/home/guestuser1/ConvNext_femto/fold/{fold_name}/{fold_name}_model/ConvNext_{job_index}.pt')
...
pred, auc_score = evaluate(ConvNext, valid_loader)
print(f'ConvNext AUC: {auc_score:.4f}')



pred.to_csv(f'/home/guestuser1/ConvNext_femto/fold/{fold_name}/{fold_name}_pred/{job_index}_pred.csv', index=False)
