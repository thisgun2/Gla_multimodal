...
print(job_index,"B_scan","VIT fold")
...
VIT = ModifiedVIT(pretrained=True).to(Device)

for param in VIT.model.parameters():
            param.requires_grad = True


optimizer_ft = optim.Adam(filter(lambda p: p.requires_grad, VIT.parameters()), eps=1e-8, lr=0.0001, weight_decay=1e-4)
...
def train_VIT(model, criterion, optimizer, scheduler, dataloaders, dataset_sizes, num_epochs=50, early_stopping_patience=5):
    since = time.time()
...
VIT = train_VIT(VIT, criterion, optimizer_ft, scheduler, dataloaders, dataset_sizes, num_epochs=50, early_stopping_patience=5)


torch.save(VIT, f'/home/guestuser1/VIT/fold/{fold_name}/{fold_name}_model/VIT_{job_index}.pt')
...
pred, auc_score = evaluate(VIT, valid_loader)
print(f'VIT AUC: {auc_score:.4f}')



pred.to_csv(f'/home/guestuser1/VIT/fold/{fold_name}/{fold_name}_pred/{job_index}_pred.csv', index=False)
