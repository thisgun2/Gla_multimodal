...
print("B scan : ",job_index, "xcit fold0")
...
class Modifiedxcit(nn.Module):
    def __init__(self, model_name="xcit_nano_12_p16_224", pretrained=True):
        super(Modifiedxcit, self).__init__()
...
xcit = Modifiedxcit(pretrained=True).to(Device)




for param in xcit.model.parameters():
            param.requires_grad = True


 

optimizer_ft = optim.Adam(filter(lambda p: p.requires_grad, xcit.parameters()), eps=1e-8, lr=0.0001, weight_decay=1e-4)
...
def train_xcit(model, criterion, optimizer, scheduler, dataloaders, dataset_sizes, num_epochs=50, early_stopping_patience=5):
    since = time.time()
...
xcit = train_xcit(xcit, criterion, optimizer_ft, scheduler, dataloaders, dataset_sizes, num_epochs=50, early_stopping_patience=5)


torch.save(xcit, f'/home/guestuser1/xcit/fold/{fold_name}/{fold_name}_model/xcit_{job_index}.pt')
...
pred, auc_score = evaluate(xcit, valid_loader)
print(f'xcit fold AUC: {auc_score:.4f}')



pred.to_csv(f'/home/guestuser1/xcit/fold/{fold_name}/{fold_name}_pred/{job_index}_pred.csv', index=False)
