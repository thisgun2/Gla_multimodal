...
import sys
# Accept tow command-line arguments: job_index and fold_name
job_index = sys.argv[1]
fold_name = sys.argv[2]
print(job_index,"B_scan", flush=True)
...
print("PIT fold0",job_index,"B_scan")
...
class ModifiedPIT(nn.Module):
    def __init__(self, model_name="pit_ti_224", pretrained=True):
        super(ModifiedPIT, self).__init__()
...
PIT = timm.create_model('pit_ti_224', pretrained=True)
PIT = ModifiedPIT(pretrained=True).to(Device)

for param in PIT.parameters():
            param.requires_grad = True


optimizer_ft = optim.Adam(filter(lambda p: p.requires_grad, PIT.parameters()), eps=1e-8, lr=0.0001, weight_decay=1e-4)
...
def train_PIT(model, criterion, optimizer, scheduler, dataloaders, dataset_sizes, num_epochs=50, early_stopping_patience=5):
    since = time.time()
...
PIT = train_PIT(PIT, criterion, optimizer_ft, scheduler, dataloaders, dataset_sizes, num_epochs=50, early_stopping_patience=5)


torch.save(PIT, f'/home/guestuser1/PIT/fold/{fold_name}/{fold_name}_model/PIT_{job_index}.pt')
...
pred, auc_score = evaluate(PIT, valid_loader)
print(f'PIT fold AUC: {auc_score:.4f}')



pred.to_csv(f'/home/guestuser1/PIT/fold/{fold_name}/{fold_name}_pred/{job_index}_pred.csv', index=False)
